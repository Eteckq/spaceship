...
function initModelShader() {
  ...
  modelShader.modelMatrixUniform = gl.getUniformLocation(
    modelShader,
    "uModelMatrix"
  );
  modelShader.viewMatrixUniform = gl.getUniformLocation(
    modelShader,
    "uViewMatrix"
  );
  modelShader.projMatrixUniform = gl.getUniformLocation(
    modelShader,
    "uProjMatrix"
  );
...
Model.prototype.move = function (x, y) {
  ...
  this.rotation = this.position[0] * 0.2; // permet de tourner autour de l'axe Y
...
Model.prototype.sendUniformVariables = function () {
  ...
  if (this.loaded) {
    ...
    gl.uniformMatrix4fv(
      modelShader.modelMatrixUniform,
      false,
      this.modelMatrix
    );
